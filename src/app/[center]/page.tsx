...
import { getCenterInfoByCenterId, getTrainersByCenter, getReviewsByCenter, getFacilitiesByCenter } from '@/lib/sanityData';
...
export default async function CenterPage({ params }: CenterPageProps) {
  ...
  // 병렬로 트레이너, 리뷰, 시설 데이터 가져오기 (이미 변환된 상태)
  const [trainers, reviews, facilities] = await Promise.all([
    getTrainersByCenter(centerInfo.centerId),
    getReviewsByCenter(centerInfo.centerId),
    getFacilitiesByCenter(centerInfo.centerId)
  ]);
